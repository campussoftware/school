<?php
    ...
    define("SESSIONPATH", 'studentportal');
    define("APIURL", "http://192.168.0.150/school/api/");
    function getStudentProfileDetails($requestedData)
    {
        $output=array();
        $curlCall=new Core_CurlCall();
        $curlCall->setUrl(APIURL."student_admission/validateAdmission");
        $curlCall->setPostData("admissionno",$requestedData['admissionno']);
        $curlCall->setPostData("password",$requestedData['password']);
        $curlCall->setReturnTransfer(true);
        $studentData=$curlCall->callCurl();
        $studentDataArray=convertJsonToArray($studentData);
        $data=convertJsonToArray($studentDataArray['data']);
        if($studentDataArray['status']=="success")
        {
                session_start();
                if($studentDataArray['message']=="loginsuccess")
                {
                    $_SESSION[SESSIONPATH]['name']=$data['name'];
                    $_SESSION[SESSIONPATH]['id']=$data['id'];
                    $_SESSION[SESSIONPATH]['admission_no']=$data['admission_no'];
                    $_SESSION[SESSIONPATH]['academicyear_code']=$data['academicyearcode'];
                    $_SESSION[SESSIONPATH]['academicyear']=$data['academicyear'];
    ...
    function getStudentFullDetails()
    {
        ...
        $curlCall=new Core_CurlCall();
        $curlCall->setUrl(APIURL."student_admission/getstudentDetails/".$studentId);
        $curlCall->setPostData("academicyear", $academicyear);
        $curlCall->setReturnTransfer(true); 
        $studentData=$curlCall->callCurl();       
        $studentDataArray=convertJsonToArray($studentData);
        $studentDataArray;
        return $studentDataArray['data'];
    }
    function getStudentFeeDetails()
    {
        ...
        $academicyear=$_SESSION[SESSIONPATH]['academicyear_code'];
        $curlCall=new Core_CurlCall();
        $curlCall->setUrl(APIURL."student_admission/feeDetails/".$studentId);
        $curlCall->setPostData("academicyear", $academicyear);
        $curlCall->setReturnTransfer(true);
        $studentData=$curlCall->callCurl();
        $studentDataArray=convertJsonToArray($studentData);
        $studentData = convertJsonToArray($studentDataArray['data']);
        return $studentData;
    }
    function getTeacherListDetails()
    {
        $studentId=$_SESSION[SESSIONPATH]['id'];
        $academicyear=$_SESSION[SESSIONPATH]['academicyear_code'];
        $curlCall=new Core_CurlCall();
        $curlCall->setUrl(APIURL."staff_staff_subjects/getStaffList/".$studentId);
        $curlCall->setPostData("academicyear", $academicyear);
        $curlCall->setReturnTransfer(true); 
        $teacherData=$curlCall->callCurl();
        $teacherDataArray=convertJsonToArray($teacherData);
        $teacherData = convertJsonToArray($teacherDataArray['data']);
        return $teacherData;
    }

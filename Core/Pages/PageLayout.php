...
class PageLayout extends Language {
    ...
    public function loadLayout($filename, $duplicateLoad = 0, $returnFile = 0) {
        ...
        global $rootObj;
        $ws = $rootObj;
        $templateRootPath = $ws->documentRoot . "pages".DIRECTORY_SEPARATOR;

        if ($actionRequestFrom == "admin") {
            $templateRootPath .= "adminhtml".DIRECTORY_SEPARATOR;
        } else {
            $templateRootPath .= "frontend".DIRECTORY_SEPARATOR;
        }
        $currentnode = is_object($controllerObj) ? $controllerObj->_nodeName : "";
        $includepath = "";
        if ($currentnode) {
            $module = $controllerObj->_currentNodeModule;
            $module = \Core::convertStringToFileName($module);
            if (strpos($filename, '::') !== false) {
                if ($actionRequestFrom == "admin") {
                   $targetFile = \Core::getTempAdminThemePath() . "template".DIRECTORY_SEPARATOR. str_replace(array("::","/"),array(DIRECTORY_SEPARATOR,DIRECTORY_SEPARATOR),$filename);                  
                } else {
                    $targetFile = \Core::getTempFrontendThemePath() . "template".DIRECTORY_SEPARATOR. str_replace(array("::","/"),array(DIRECTORY_SEPARATOR,DIRECTORY_SEPARATOR),$filename);                                      
                }
            } else {
                if ($actionRequestFrom == "admin") {
                    $targetFile = \Core::getTempAdminThemePath() . "template" . DIRECTORY_SEPARATOR . $filename;
                } else {
                    $targetFile = \Core::getTempFrontendThemePath() . "template" . DIRECTORY_SEPARATOR . $filename;
                }
            }
            ...
            if (strpos($filename, '::') !== false) {
                if ($flag == 0 && $module) {
                    $filenamePath = $templateRootPath . $ws->themeName . DIRECTORY_SEPARATOR."template".DIRECTORY_SEPARATOR . str_replace(array("::","/"),array(DIRECTORY_SEPARATOR,DIRECTORY_SEPARATOR),$filename); 
            ...
            if ($flag == 0) {
                $filenamePath = $templateRootPath . $ws->themeName . DIRECTORY_SEPARATOR."template".DIRECTORY_SEPARATOR . $filename;
